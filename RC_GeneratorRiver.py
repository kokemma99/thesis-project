...
def settling(MP_density_kg_m3, MP_radius_m, comp_depth_m, settlingMethod, compartment, MP_CSF):
    ...
    if compartment == "4":
        ...
        elif settlingMethod == 'CalcFiber':
             wx = 6*10**-7
             Re = (wx * (MP_radius_m*2)) / mu_w_21C_kg_ms
             C_d_s = 4.7 / math.sqrt(Re) + math.sqrt(MP_CSF)
             vSet_m_s = math.sqrt((4/3)*((MP_radius_m*2)/C_d_s)*abs((MP_density_kg_m3-density_w_21C_kg_m3)/density_w_21C_kg_m3)*g_m_s2)
        else: 
...
def rising(MP_density_kg_m3, MP_radius_m, comp_depth_m, settlingMethod, compartment, MP_CSF):
    #rising calculations
...
def heteroagg(alpha, MP_radius_m, SPM_radius_m, MP_density_kg_m3, SPM_density_kg_m3, SPM_concNum_part_m3, G, T_K, compartment, MP_CSF):
    ...
    if compartment == "4":
        ...
        Re = (wx * (MP_radius_m*2)) / mu_w_21C_kg_ms
        C_d_s = 4.7 / math.sqrt(Re) + math.sqrt(MP_CSF)
        MP_vSet_m_s = math.sqrt((4/3)*((MP_radius_m*2)/C_d_s)*abs((MP_density_kg_m3-density_w_21C_kg_m3)/density_w_21C_kg_m3)*g_m_s2)
